...
>
> **✅ Validation 2026-06-10 (follow-up fixes, 322/322 tests green):** All five remaining items resolved — (1) **Declined-repair silent failure fixed**: `offerRepair()` now accepts an `onDecline` callback; all 5 callsites (worker source/target, sync-fallback source/target, format/minify buttons covering base pane) pass `showParseError`/`showError` as the fallback so cancelling the repair dialog always surfaces the standard error banner with line/col + Jump. (2) **Parity test added**: `tests/index-parity.test.js` strips `FEEDBACK:START/END` blocks and the one CSP connect-src line from `index.html`, then asserts byte equality with `json_compare.html`; test fails if the files drift anywhere outside those markers. `index.html` FEEDBACK blocks are marked with `/* FEEDBACK:START/END */` (CSS/JS) and `<!-- FEEDBACK:START/END -->` (HTML). (3) **Mobile 390 px pass**: `appbar-inner .icon-btn` gets `flex: none` so the 4 appbar icon buttons hold their 36×36 size; `.titles` gets `min-width: 0; overflow: hidden` and `h1` gets `white-space: nowrap; text-overflow: ellipsis` so the page title truncates rather than overflowing the bar. All other P1-7 breakpoints (pane-tools scroll row, icon-only secondary toolbar, full-width filter inputs, static sticky headers ≤820 px) confirmed intact. (4) **`index.html` regenerated** from `json_compare.html` + feedback delta; 322/322 tests green incl. both parity tests; `node --check` clean on both files. (5) **Repo hygiene**: `package-lock.json` and all planning docs committed.
